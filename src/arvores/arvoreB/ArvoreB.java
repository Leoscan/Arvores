...
public class ArvoreB extends Arvore {
    ...
    public void Inserir(int k) {
        ...
        if (buscarNaArvore(raiz, k) == null) { //só insere se não houver, para evitar duplicação de chaves
    ...
    public void Buscar(int k) {
        System.out.println(buscarNaArvore(raiz, k));
    }

    private No buscarNaArvore(No X, int k) {
        int i = 1;
        // Procura até não estourar o tamanho e até o valor não ser maior que o procurado.
        while ((i <= X.getN()) && (k > X.getChave().get(i - 1))) { 
            i++;
        }
        // Se o tamanho não tiver excedido e for o valor procurado...
        if ((i <= X.getN()) && (k == X.getChave().get(i - 1))) {
            return X;
        }
        // Se não foi encontrado, procura no filho se não for folha.
        if (X.isFolha()) {
            return null; // Elemento não encontrado.
        } else {
            return buscarNaArvore(X.getFilho().get(i - 1), k);
        }
    }
    ...
    public void Remover(int k) {
        ...
        if (buscarNaArvore(this.raiz, k) != null) {
            //N é o nó onde se encontra k
            No N = buscarNaArvore(this.raiz, k);
            int i = 1;
        ...
        if (no != null) {
            if (!no.isFolha()) {
                for (int i = 0; i < no.getN(); i++) {
                    percorrerEmOrdem(no.getFilho().get(i));
                    System.out.print(no.getChave().get(i) + " ");
                }
                percorrerEmOrdem(no.getFilho().get(no.getN()));
            } else {
                for (int i = 0; i < no.getN(); i++) {
                    System.out.print(no.getChave().get(i) + " ");
                }
    ...
    // Método público para iniciar o percurso em ordem na raiz
    public void Imprimir_Valores() {
        percorrerEmOrdem(raiz);
        System.out.println(); // Para adicionar uma quebra de linha no final
    }
